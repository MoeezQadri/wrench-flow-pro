...
interface InviteRequest {
  ...
  superAdminDashboardReq?: boolean; // optional, defaults to false
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (!superAdminDashboardReq && profile.organization_id !== organizationId) {
    ...
    const foundUser = usersPage?.users?.find(
      (u: any) => (u.email ?? u.user_metadata?.email) === email
    );
    ...
    console.log(
      `User ${user.email} invited ${email} with role ${role} to organization ${organizationId}`
    );
