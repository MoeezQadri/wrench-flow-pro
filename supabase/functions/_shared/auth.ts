...
const USER_JWT_SECRET =
  Deno.env.get('USER_JWT_SECRET') || 'user-jwt-secret-secure-2024';
...
export async function verifyJWT(superadmin_token) {
...
export async function verifyUserJWT(token) {
  ...
  try {
    ...
    if (user) {
      // Return the user object and the user role for further checks if needed
      return {
        ...user,
