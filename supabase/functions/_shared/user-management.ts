...
export async function getOrganizations() {
  ...
  const { data: organizations, error } = await supabaseAdmin
    .from('organizations_with_profiles')
    .select('*')
    .order('created_at', {
      ascending: false,
    });
...
export async function getUsers() {
  const supabaseAdmin = await getSupabaseAdmin();
  try {
    // Try to use the profiles_with_email view first
...
export async function getInactiveUsers(daysInactive = 90) {
  const supabaseAdmin = await getSupabaseAdmin();
  // Get inactive users using the RPC function
  const { data: users, error } = await supabaseAdmin.rpc('get_inactive_users', {
...
export async function cleanUserData(userId) {
  const supabaseAdmin = await getSupabaseAdmin();
  // Clean user data using the RPC function
  const { data, error } = await supabaseAdmin.rpc('clean_user_data', {
    user_id: userId,
  });
  if (error) {
...
export async function enableUserWithoutConfirmation(userId) {
  const supabaseAdmin = await getSupabaseAdmin();
  try {
...
export async function updateUser(input) {
  const { userId, email, name, role } = input;
  const supabaseAdmin = await getSupabaseAdmin();

  /* ----------------------------------------------
   * 1) Update auth.users (email + name)
   * ---------------------------------------------- */
  const { data: authUser, error: authError } =
    await supabaseAdmin.auth.admin.updateUserById(userId, {
      email, // updates the user's email
      user_metadata: { name }, // stores name safely in metadata
    });
