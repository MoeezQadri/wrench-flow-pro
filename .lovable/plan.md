...
## 1. "Completed" invoices and payments

Today the invoice list hides the Edit button for invoices marked **Paid** *and* **Completed**, so a completed invoice can never be opened, and the Add Payment box only exists inside the invoice edit screen. That is why the payment button appears to do nothing on those invoices. There are 7 completed invoices right now.

Changes:
- **Completed** means the work is finished but money is still due. Completed invoices become editable again and can take payments.
...
- Estimates and declined quotes keep taking no payments (unchanged).

## 2. Add Payment on an invoice

- Keep the safety rules (amount above zero, cannot exceed the amount still due, no double-click double-entry) but make the refusal message say exactly what is wrong and how much is still due.
