...
- New `src/components/vendor/VendorBillsSection.tsx` and `src/components/vendor/PayVendorDialog.tsx`, using `getPayablesByVendor` from `usePayables` and calling `markPayableAsPaid` per allocated bill. `src/components/payable/AddBillDialog.tsx` is dropped along with its trigger on the page.
...
- Verification: typecheck, then an end-to-end pass once the changes are done — expense → bill → partial payment → full payment, part purchase → vendor bill, vendor lump payment across two bills, invoice creation with workshop and custom items, paid-status validation and payment preservation, estimate conversion, and Payable Management / expense / report / dashboard figures agreeing, plus owner, admin, finance and technician access checks.
